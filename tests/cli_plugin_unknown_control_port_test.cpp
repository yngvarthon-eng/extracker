...
int main() {
  ...
  if (pluginPath.empty() || !std::filesystem::exists(pluginPath)) {
    std::cerr << "LV2 CLI unknown-control-port test plugin binary not found" << '\n';
    return 1;
  }

  const auto tmpRoot = std::filesystem::temp_directory_path() / "extracker_cli_plugin_unknown_control_port_test";
  ...
  if (setenv("LV2_PATH", tmpRoot.string().c_str(), 1) != 0) {
    std::cerr << "Failed to set LV2_PATH for CLI unknown-control-port test" << '\n';
    return 1;
  }
  ...
  if (pipe == nullptr) {
    std::cerr << "Failed to spawn CLI plugin unknown-control-port command" << '\n';
    return 1;
  }
  ...
  if (status != 0) {
    std::cerr << "CLI plugin unknown-control-port command exited non-zero: " << status << '\n';
    std::cerr << output << '\n';
    return 1;
  }

  const bool sawUnknownInputPort = output.find("Unknown control input port: nope") != std::string::npos;
  const bool sawUnknownPort = output.find("Unknown control port: nope") != std::string::npos;

  if (!sawUnknownInputPort || !sawUnknownPort) {
    std::cerr << "Missing expected unknown control port output markers" << '\n';
    std::cerr << output << '\n';
    return 1;
  }
