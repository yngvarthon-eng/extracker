...
bool appendGuiOnlyTailTokens(std::string& text) {
  if (!text.empty() && text.back() != '\n') {
    text.push_back('\n');
  }

  text += "MIDI_EDITOR_CC_MAP 74 71 1 2 7 10 11 74\n";
  text += "CHANNEL_INSTRUMENTS 1 2 3 4 5 6 7 8\n";
  text += "CHANNEL_MUTED 0 1 0 1 0 1 0 1\n";
  return true;
}
...
int main() {
  ...
  const fs::path basePath = fs::temp_directory_path() / "extracker_cli_gui_tail_token_compat_test";
  ...
  if (!replacePatternSwingLine(firstText, "PATTERN_SWING 71 64 52")) {
    std::cerr << "Saved file missing PATTERN_SWING line" << '\n';
    return 1;
  }
  appendGuiOnlyTailTokens(firstText);
  ...
  if (!containsLine(secondText, "PATTERN_SWING 71 64 52")) {
    std::cerr << "Round-tripped file missing expected PATTERN_SWING values" << '\n';
    std::cerr << secondText << '\n';
    return 1;
  }
