...
int main() {
  ...
  if (pluginPath.empty() || !std::filesystem::exists(pluginPath)) {
    std::cerr << "LV2 CLI plugin-set test plugin binary not found" << '\n';
    return 1;
  }

  const auto tmpRoot = std::filesystem::temp_directory_path() / "extracker_cli_plugin_set_test";
  ...
  if (setenv("LV2_PATH", tmpRoot.string().c_str(), 1) != 0) {
    std::cerr << "Failed to set LV2_PATH for CLI plugin set test" << '\n';
    return 1;
  }
  ...
  if (pipe == nullptr) {
    std::cerr << "Failed to spawn CLI plugin set command" << '\n';
    return 1;
  }
  ...
  if (status != 0) {
    std::cerr << "CLI plugin set command exited non-zero: " << status << '\n';
    std::cerr << output << '\n';
    return 1;
  }
  ...
  if (!sawScan || !sawLoad || !sawAssign || !sawSet) {
    std::cerr << "Missing expected plugin set output markers" << '\n';
    std::cerr << output << '\n';
    return 1;
  }
