...
int main() {
  ...
  std::cout << "MIDI commands: midi on, midi off, midi status, midi thru <on|off>, midi instrument <index>, midi learn <on|off|status>, midi map <ch> <instr|clear>, midi map <status|clear all>, midi transport <on|off|status|timeout|lock>, midi clock <help|sources|autoconnect|diagnose>" << '\n';
